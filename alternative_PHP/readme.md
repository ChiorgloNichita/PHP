...
Тест может проходить любой пользователь анонимно.
...
1. Загрузите файлы на сервер с поддержкой PHP.
2. Убедитесь, что файл `data.json` доступен для записи.
3. Перейдите на главную страницу `index.php` для начала теста.
4. После прохождения теста результаты будут отображены на странице `result.php`.
